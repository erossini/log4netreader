...
using Inffectus.Design.Commands;
...
namespace Inffectus.Infrastructure.Ui
{
    public class Presenters
	{
		...
		{
			var type = Assembly.GetExecutingAssembly().GetType("Inffectus.Features." + name + ".Presenter");
			if (type == null)
			...
			if (args != null && args.Length > 0)
			{
				...
				if(init == null)
					throw new InvalidOperationException("Presnter to be shown we argument, but not initialize method found");
        ...
        private static void WireMenuItems(IPresenter presenter)
        {
            var presenterType = presenter.GetType();
            var methodsAndMenuItems =
                    from method in GetCallParameterlessMethods(presenterType)
                    let elementName = method.Name.Substring(4)
                    let matchingControl = LogicalTreeHelper.FindLogicalNode(presenter.View, elementName) as MenuItem
                    let fact = presenterType.GetProperty("Can" + elementName)
                    where matchingControl != null
                    select new { method, fact, menuItem = matchingControl };

            foreach (var matching in methodsAndMenuItems)
            {
                var action = (Action)Delegate.CreateDelegate(typeof(Action),
                                                              presenter, matching.method);
                Fact fact = null;
                if (matching.fact != null)
                    fact = (Fact)matching.fact.GetValue(presenter, null);
                matching.menuItem.Command = new DelegatingCommand(action, fact);
            }
        }
        ...
        private static IEnumerable<MethodInfo> GetTypeMethods(Type type, string startingWith)
        {
            return from method in type.GetMethods(BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Instance)
                   where method.Name.StartsWith(startingWith)
                   select method;
        }

        private static IEnumerable<MethodInfo> GetCallParameterlessMethods(Type type)
        {
            return GetTypeMethods(type, "Call");
        }

        private static IEnumerable<MethodInfo> GetActionMethods(Type type)
        {
            return GetTypeMethods(type, "On");
        }
